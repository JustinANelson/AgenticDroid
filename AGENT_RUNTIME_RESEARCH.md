...
- Added `libnpm_wrapper.so` and `libnpx_wrapper.so` as source-controlled shell scripts in
  `jniLibs/arm64-v8a`. They invoke the already native-lib-packaged `node`; npm's JS entry
